...
inline int parseField(std::string_view block, std::string_view key) {
	...
	while (searchFrom < block.size()) {
		...
		if (afterKey < block.size() && (std::isalnum(static_cast<unsigned char>(block[afterKey])) || block[afterKey] == '_')) {
			searchFrom = afterKey;
			continue;
		}
		pos = afterKey;
...
inline bool parseOutfit(std::string_view content, Outfit& outfit) {
	...
	if ((val = parseField(block, "lookMountHead")) >= 0) {
		outfit.lookMountHead = val;
	}
	if ((val = parseField(block, "lookMountBody")) >= 0) {
		outfit.lookMountBody = val;
	}
	if ((val = parseField(block, "lookMountLegs")) >= 0) {
		outfit.lookMountLegs = val;
	}
	if ((val = parseField(block, "lookMountFeet")) >= 0) {
		outfit.lookMountFeet = val;
	}
	return hasBaseLook;
}
