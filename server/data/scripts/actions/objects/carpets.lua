...
function carpets.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local carpetId = carpetItems[itemId]
	if not carpetId then
		return false
	end
	...
	if not tile then
		return fail(player, "You may use this only inside a house.")
	end
	...
	if not house:isInvited(player) then
		return fail(player, "You cannot modify items in another person's house.")
	end
...
for id in pairs(carpetItems) do
	carpets:id(id)
end
