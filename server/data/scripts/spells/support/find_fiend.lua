...
local function pickClosestFiendish(creature)
	...
	for _, monster in ipairs(Game.getMonsters()) do
		if monster and monster:isFiendish() then
			local distance = monster:getPosition():getDistance(playerPosition)
...
function spell.onCastSpell(creature, variant)
	local target = pickClosestFiendish(creature)
	if not target then
	...
	message = string.format("The monster %s.", message)
