...
local missingTargetBotConfigWarned = false

local function CaveBotConfigParse()
    local configs = storage._configs and storage._configs.targetbot_configs
    local name = configs and configs.selected
    if not name then
        if not missingTargetBotConfigWarned then
            warn("[vBot] Please create a new TargetBot config and reset bot")
            missingTargetBotConfigWarned = true
        end
        return nil
    end

    missingTargetBotConfigWarned = false
    local file = configDir .. "/targetbot_configs/" .. name .. ".json"
    if not g_resources.fileExists(file) then return nil end

    local data = g_resources.readFileContents(file)
    local config = Config.parse(data)
    return config and config.looting
end
...
function CaveBot.GetLootItems()
    local config = CaveBotConfigParse()
...
function CaveBot.PingDelay(multiplayer)
    ...
    if currentPing and currentPing > 150 then -- in most cases ping above 150 affects CaveBot
...
function CaveBot.WithdrawItem(id, amount, fromDepot, destination)
    ...
    if not destination then
        for i, container in pairs(getContainers()) do
            local name = container:getName():lower()
            if container:getCapacity() > #container:getItems() and not name:find("quiver") and not name:find("depot") and not name:find("loot") and not name:find("inbox") then
