...
Panels.Health = function(parent)
  ...
  context.macro(25, function()
    local config = context.storage[panelName]
    if not config.enabled or config.text:len() == 0 then return end

    local healthPercent = context.hppercent()
    if config.min <= healthPercent and healthPercent <= config.max then
      if context.saySpell(config.text, 500) then
        context.delay(200)
      end
    end
  end)
...
Panels.HealthItem = function(parent)
  ...
  context.macro(25, function()
    local config = context.storage[panelName]
    if not config.enabled or config.item < 100 then return end

    local healthPercent = context.hppercent()
    if config.min <= healthPercent and healthPercent <= config.max then
      if context.useRune(config.item, context.player, 500) then
        context.delay(300)
      end
    end
  end)
...
Panels.Mana = function(parent)
  ...
  context.macro(25, function()
    ...
    local manaPercent = context.manapercent()
    if config.min <= manaPercent and manaPercent <= config.max then
      if context.useRune(config.item, context.player, 500) then
        context.delay(300)
      end
    end
